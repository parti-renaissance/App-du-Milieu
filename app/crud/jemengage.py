...
def downloads_ratio(
    ...
    df_nat.rename(
        columns={
            'downloadsPer1000': 'nationalPer1000'},
        inplace=True)
    ...
    if not df.empty:
        ...
        df = df.set_index('date').reindex(s_date).rename_axis('date')
        # fill unique user to 0
        df['downloadsPer1000'] = df['downloadsPer1000'].fillna(
            method='ffill').round(3)
        df_nat['nationalPer1000'] = df_nat['nationalPer1000'].fillna(
            method='ffill').round(3)
                    ...
                    JecouteDataSurvey.survey)) .filter(
                        JecouteDataSurvey.postal_code != '') .join(
                            ...
                            GeoCity.postal_code.like(
                                ...
                                '%')) .join(GeoDepartment) .join(GeoRegion) .filter(
                                    GeoRegion.code == zone.code) .filter(
